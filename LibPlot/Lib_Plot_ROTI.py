...
def load_data_roti(File_name, all_data = {}, data_type = "ROTI" , start_week = 0):
    soweek_last = 0
    w_last = start_week
    if os.path.exists(File_name):
        with open(File_name,'rt') as f:
            for line in f:
                ...
                all_data[data_type][soweek][sat] = roti
...
def plot_roti_G_E_C(Plot_Data={}, Ylim=1.0, XlabelSet = [], Delta_date = 1, Show=True, Start = [], End = [], Legend = True):
...
def plot_roti_multi_site_GEC(Plot_Data={}, Ylim=1.0, XlabelSet = [], Delta_date = 1, Show=True, Start = [], End = [], Legend = True):
    ...
    figP,axP = plt.subplots(plot_num,1,figsize=(12,3*plot_num),sharey=True,sharex=True)
    # box = axP[].get_position()
    # axP.set_position([box.x0, box.y0 + box.height*0.13, box.width, box.height*0.9])
    #=== Plot ===#
    index_plot = 0
...
def Plot_timeseries_ROTI(File_info=[],Start=[],End=[],Plot_type=[],Ylim = 1.0,Delta_xlabel = 4,Delta_date = 3,Save_dir="",Show=True,All=False,Time_type = "GPST"):
            ...
            for cur_time in all_data[cur_mode].keys():
                ...
                if ((plot_time >= Start[3] and plot_time <= (Start[3]+duration_time)) or All):
                    for cur_sat in all_data[cur_mode][cur_time].keys():
                        ...
                        PLOT_ALL[cur_mode][cur_sat].append(all_data[cur_mode][cur_time][cur_sat])
    ...
    if len(Plot_type) == 1:
        if "G_E_C" in Plot_type[0]:
            plot_roti_G_E_C(Plot_Data=PLOT_ALL["ROTI"], Ylim=Ylim, XlabelSet = [XLabel,XTick], Delta_date = Delta_date, Start = Start, End = End, Show=Show)
